...
class TelegramAdapter implements PostDeployAdapterInterface
{
    ...
    public function run(Deploy $deploy) : bool
    {
        ...
        $telegram->sendMessage([
          ...
          'text'                     => '`'.$deploy->getVersion().'` was deployed on *'.gethostname().'*'."\n".'['.subst($deploy->strategy->getCommitHash(), 0, 8).']('.$deploy->strategy->getCommitUrl().') `'.$deploy->strategy->getCommitMessage().'` by [@'.$deploy->strategy->getCommitUsername().'](https://github.com/'.$deploy->strategy->getCommitUsername().')',
        ]);
